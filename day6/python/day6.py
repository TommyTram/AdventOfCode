#!/usr/bin/env python3
from os.path import dirname, realpath, join
import time, pytest

def transform_input(input_):
  # custom transform for the day
  new_input = list()
  lines = input_.splitlines()
  answers = dict({'nr_people':0})

  for line in lines:

    if line == '':
      new_input.append(answers)
      answers = dict({'nr_people':0})
      continue
    
    answers['nr_people'] += 1
    for question in line:
      personal_answer = answers.get(question) + 1 if answers.get(question) else 1
      answers.update({question:personal_answer})
  
  new_input.append(answers)

  return new_input

def read_input(file_name = '../data/input.txt'):
  dir_path = dirname(realpath(__file__))
  with open(join(dir_path,file_name), 'r') as f:
    input_ = transform_input(f.read())

  return input_

def solve_part1(input_):
  count = 0
  for group in input_:
    count += len(group.keys())
  return count


def solve_part2(input_):
  count = 0
  for group in input_:
    summary = [k for k,v in group.items() if v == group.get('nr_people') and k != 'nr_people']
    count += len(summary)
    

  return count


def main():
  
  input_ = read_input()

  t0 = time.time()
  part1 = solve_part1(input_)
  time_part1 = round((time.time()-t0)*1e3)
  print(f'Solution to part one: {part1} (time taken {time_part1}[ms])')

  t0 = time.time()
  part2 = solve_part2(input_)
  time_part2 = round((time.time()-t0)*1e3)
  print(f'Solution to part two: {part2} (time taken {time_part2}[ms])')


if __name__ == '__main__':
  main()

@pytest.mark.parametrize("input1,output1", [
  ('../data/test_input0.txt', 1)
])

def test1(input1,output1):
  assert solve_part1(read_input(input1)) == output1

@pytest.mark.parametrize("input2,output2", [
  ('../data/test_input0.txt', 1)
])

def test2(input2,output2):
  assert solve_part2(read_input(input2)) == output2